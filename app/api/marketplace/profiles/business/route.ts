...
import { getBusinessByUser } from "@/lib/marketplace/access";
...
export async function GET(request: Request) {
  ...
  if (user.role !== "business") return jsonError("Business account required", 403);
  const profile = await getBusinessByUser(user.id);
...
export async function PATCH(request: Request) {
  ...
  if (typeof data.name === "string" && data.name.trim()) {
    profile.name = data.name.trim().slice(0, 160);
  }
  if (typeof data.slug === "string" && data.slug.trim() && data.slug !== profile.slug) {
    profile.slug = await uniqueSlug(BusinessProfile, "slug", data.slug);
  }
  if (typeof data.category === "string") profile.category = asString(data.category, 80);
  if (typeof data.about === "string") profile.about = data.about.slice(0, 3000);
